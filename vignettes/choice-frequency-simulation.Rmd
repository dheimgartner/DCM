...
m <- maxLik::maxLik(loglik, start = c(alpha = 0, a1 = -1, a2 = 1))
summary(m)
```
...
loglik <- function(param) {
  ...
  n_p_inf <- rep(Inf, n)
  pt0 <- 1 - pnorm(gammaX)
  pt1_j1 <- pmnorm(cbind(gammaX, a1 - alphaZ), varcov = sigma) # below
  pt1_j2 <- pmnorm(cbind(gammaX, a2 - alphaZ), varcov = sigma) - pt1_j1 # between
  pt1_j3 <- pmnorm(cbind(gammaX, -(a2 - alphaZ)), varcov = sigma) # above
