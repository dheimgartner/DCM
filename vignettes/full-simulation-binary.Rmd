...
The probability that the agent chooses outcome $y$ can be expressed as an expectation (integral over the random part) CITE:
...
Clearly, our `loglik` function makes sense: Evaluated at the true parameters, we get the highest likelihood (which matches the non-simulated likelihood from the chapter before):
...
Important remark: The line `P_1n <- pmax(pmin(P_1n, 1 - epsilon), epsilon)` is needed because the simulated integral (depending on the parameter values passed to it) yields $P_1n = 0$ which yields `-Inf` when taking the log. Similarly, $P_1n = 1$ would yield $P_0n = 0$ and thus the same problem. Therefore we add (or subtract) a very small value `epsilon` to 0 (from 1).

Trying to estimate (again using `maxLik`) does not seem to work. Why?
